...
1. Write a function `doubleAndAdd` that takes two numbers and returns the sum of **double** the first number and the second number.

```js
const doubleAndAdd = function (num1, num2) {
  // TODO: your code here
};

// Examples
doubleAndAdd(2, 3); // => 7  (2*2 + 3)
doubleAndAdd(5, 10); // => 20
``` 
2. Write a function `average` that accepts three numbers and returns their average.
...
average(10, 20, 30); // => 20
```
3. Write a function `sumOfSquares` that takes two numbers and returns the sum of their squares.
...
4. Write a function `personalGreeting` that takes two arguments — `name` and `timeOfDay` — and returns a message like:

`"Good morning, John!" or "Good evening, Sara!".`

```js
const personalGreeting = function (name, timeOfDay) {
  // TODO: your code here
};

// Examples
personalGreeting("John", "morning"); // => "Good morning, John!"
personalGreeting("Sara", "evening"); // => "Good evening, Sara!"
```

5. Write a function `formatFullName` that takes two parameters `firstName` and `lastName` and returns a formatted full name with a space in between and the first letter of each capitalized.

```js
const formatFullName = function (firstName, lastName) {
  // TODO: your code here
};

// Examples
formatFullName("john", "doe"); // => "John Doe"
formatFullName("mark", "SMITH"); // => "Mark Smith"
``` 
