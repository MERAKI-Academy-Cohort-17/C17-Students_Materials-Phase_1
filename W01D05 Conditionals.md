...
5. Write a function `positiveOrNegative` that accepts one argument `number` and returns a string saying whether the number is positive, negative, or if it is neutral number like zero.
   ...
   ```
### Concept Review 
1. Write a function that accepts two numbers and returns the larger one.
...
maxOfTwo(7, 7);  // => 7
``` 
2. Write a function that accepts `age` and `licenseYears`.
A person can drive only if they are 18 or older and have had a license for at least one year.
```js
const canDrive = function (age, licenseYears) {
  // TODO
};
...
```
3. Write a function that accepts three numbers: `num`, `min`, and `max`.
Return true if num is between the two limits (inclusive).
...
```
4. Write a function that accepts `age` and `parentPermission` (a boolean).
A person can watch the movie if they are 18 or older or have parent permission.

```js 
const canWatchMovie = function (age, parentPermission) {
  // TODO
};

canWatchMovie(20, false); // => true
canWatchMovie(15, true);  // => true
canWatchMovie(15, false); // => false


```
5. Write a function that accepts two boolean values.
Return `true` only if exactly one of them is true (not both).

```js
const truthChecker = function (a, b) {
  // TODO
};

truthChecker(true, false);  // => true
truthChecker(false, true);  // => true
truthChecker(true, true);   // => false
truthChecker(false, false); // => false
